# A Node.js CannabisCoin Client!

![cannabiscoin](http://i.imgur.com/xTW9sv0.png)

node-cannabiscoin is a cannabiscoin client for Node.js. It is a fork of the excellent Kapitalize Bitcoin Client (now removed from GitHub) intended for use with cannabiscoin. The purpose of this repository is:

* Provide a one-stop resource for the Node.js developer to get started with cannabiscoin integration.
* Prevent would-be cannabiscoin web developers worrying whether a Bitcoin client will work out of the box.
* Promote Node.js development of cannabiscoin web apps.
* Identify and address any incompatibilities with the cannabiscoin and Bitcoin APIs that exist now and/or in the future.

## Dependencies

You'll need a running instance of [cannabiscoind](https://github.com/esotericizm/cannabiscoin) to connect with. 

Then, install the node-cannabiscoin NPM package.

`npm install git://github.com/esotericizm/node-cannabiscoin/`

## Examples

Some code examples follow below, but for more complete examples, see [these snippets](https://github.com/esotericizm/cannabiscoin-code-snippets), or this [wallet app](comingsoon...) which was created to to test this module.

```js
var cannabiscoin = require('node-cannabiscoin')()

cannabiscoin.auth('myusername', 'mypassword')

cannabiscoin.getDifficulty(function() {
...
```js
cannabiscoin.getNewAddress(function(err, address) {
...
```js
cannabiscoin.exec('getNewAddress')

cannabiscoin.exec('getbalance', function(err, balance) {
...
```js
cannabiscoin.set('host', '127.0.0.1')
```
...
<td> <b>Currently only available on testnet</b> Add a nrequired-to-sign multisignature address to the wallet. Each key is a cannabiscoin address or hex-encoded public key. If [account] is specified, assign address to [account]. </td>
<td> N
</td></tr>
<tr>
<td> backupwallet </td>
...
<tr>
<td> dumpprivkey </td>
<td> [cannabiscoinaddress] </td>
<td> Reveals the private key corresponding to <cannabiscoinaddress< </td>
<td> Y
</td></tr>
<tr>
<td> encryptwallet </td>
...
<td> [account] </td>
<td> Returns the current cannabiscoin address for receiving payments to this account. </td>
<td> N
</td></tr>
<tr>
...
<td> getgenerate </td>
<td> </td>
<td> Returns true or false whether cannabiscoind is currently generating hashes </td>
<td> N
</td></tr>
<tr>
...
<td> getnewaddress </td>
<td> [account] </td>
<td> Returns a new cannabiscoin address for receiving payments.  If [account] is specified (recommended), it is added to the address book so payments received with the address will be credited to [account]. </td>
<td> N
</td></tr>
<tr>
...
<td> [cannabiscoinaddress] [minconf=1] </td>
<td> Returns the total amount received by <cannabiscoinaddress< in transactions with at least [minconf] confirmations. While some might consider this obvious, value reported by this only considers *receiving* transactions. It does not check payments that have been made *from* this address. In other words, this is not "getaddressbalance". Works only for addresses in the local wallet, external addresses will always show 0. </td>
<td> N
</td></tr>
<tr>
...
<tr>
<td> importprivkey </td>
<td> [cannabiscoinprivkey] [label] </td>
...
<p>To get a list of accounts on the system, execute cannabiscoind listreceivedbyaddress 0 true
</p>
</td>
<td> N
</td></tr>
...
<tr>
<td> sendfrom </td>
<td> [fromaccount] [tocannabiscoinaddress] [amount] [minconf=1] [comment] [comment-to] </td>
...
<tr>
<td> signmessage </td>
<td> [cannabiscoinaddress] [message] </td>
...
<td> stop </td>
<td> </td>
<td> Stop cannabiscoin server. </td>
<td> N
</td></tr>
<tr>
<td> validateaddress </td>
<td> [cannabiscoinaddress] </td>
<td> Return information about [cannabiscoinaddress]. </td>
<td> N
</td></tr>
<tr>
<td> verifymessage </td>
<td> [cannabiscoinaddress] [signature] [message] </td>
...
var cannabiscoin = require('cannabiscoin')({
    user:'user'
})

cannabiscoin.set('pass', 'somn')
cannabiscoin.set({port:7142})
...
You may pass an optional function `passphrasecallback` to the `node-cannabiscoin` initialization function to manage wallet unlocks. `passphrasecallback` should be a function accepting three arguments:
...
By default `cannabiscoind` exposes its JSON-RPC interface via HTTP; that is, all RPC commands are transmitted in plain text across the network! To secure the JSON-RPC channel you can supply `cannabiscoind` with a self-signed SSL certificate and an associated private key to enable HTTPS. For example, in your `cannabiscoin.conf`:

    rpcssl=1
    rpcsslcertificatechainfile=/etc/ssl/certs/cannabiscoind.crt
    rpcsslprivatekeyfile=/etc/ssl/private/cannabiscoind.pem

In order to securely access an SSL encrypted JSON-RPC interface you need a copy of the self-signed certificate from the server: in this case `cannabiscoind.crt`. Pass your self-signed certificate in the `ca` option and set `https: true` and node-cannabiscoin is secured!
    
```js
var fs = require('fs')

var ca = fs.readFileSync('cannabiscoind.crt')
...
nodeunit test/test-node-cannabiscoin.js
```

## Bounties

[cannabiscoin](http://www.cannabiscoin.com) donation address is CdmcSJ8aokHkkZS1W6KCvXiJEGw4sGkRVj

Donations in [cannabiscoin](http://www.cannabiscoin.cc) will be used for bounties. The first bounty will be awarded for creating a unit test suite. As a side note: I encourage all GitHub repository owners to post a donation address so their community can easily support development financially. 
